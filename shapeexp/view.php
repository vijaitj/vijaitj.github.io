...
<body>
	...
	for($k = 0; $k < 2; $k++)
	{
		...
		for($u_cnt = 0;$u_cnt < 3; $u_cnt++)
		{
			...
			echo "</span>";
			echo "<span class='sidenav-link-title'>Subject ".$mn_usr_dsp[$u_cnt]."</span>";
			...
			{
				echo "<li><a href='chooseshp.php?case=".$mn_case[$k]."&user=".$mn_usr[$u_cnt]."&set=".$mn_set[$s_cnt]."'>";
				echo "<span class='sidenav-link-icon'>";
				echo "<i class='material-icons'></i>";
    ...
    <div id="content">
		...
		echo "<div class='col1bot'>";
		echo "Subject View <br> <br>Subject Shape vs Model Shape: ".number_format((float)$elmz[9], 3, '.', '')."<br>Reference Shape vs Model Shape: ".number_format((float)$elmz[10], 3, '.', '')."<br>Reference Shape vs Subject Shape: ".number_format((float)$elmz[11], 3, '.', '');
		echo "</div>";
		echo "</li>";
	
		echo "<li>";
		echo "<div class='col1top'><img  id ='anim1' class ='preload' src='ScreenCaptureStore/03_15_17_03_44_21_Set_".$set."_GD_".$usr."_".$case."/".$shape."/Real.gif'> </div>";
		echo "<div class='col1bot'>";
		if($case == "Bino"){
		echo "Reference Object <br> <br>Asymmetry: ".number_format((float)$elmz[0], 3, '.', '')." <br> Compactness: ".number_format((float)$elmz[1], 3, '.', '')." <br> Modified Compactness: ".number_format((float)$elmz[2], 3, '.', '');
		}
		else{
		echo "Reference Object <br> <br>Asymmetry: ".number_format((float)$elmz[0], 3, '.', '')." <br> Compactness: ".number_format((float)$elmz[1], 3, '.', '')." <br> Modified Compactness: ".number_format(1e8*(float)$elmz[2], 2, '.', ''). "*1e-8";
		}
		echo "</div>";
		echo "</li>";
		...
		echo "<div class='col1bot'>";
		if($case == "Bino"){
		echo "Subject Reconstructed Object <br> <br>Asymmetry: ".number_format((float)$elmz[3], 3, '.', '')." <br> Compactness: ".number_format((float)$elmz[4], 3, '.', '')." <br> Modified Compactness: ".number_format((float)$elmz[5], 3, '.', '');
		}
		else{
		echo "Subject Reconstructed Object <br> <br>Asymmetry: ".number_format((float)$elmz[3], 3, '.', '')." <br> Compactness: ".number_format((float)$elmz[4], 3, '.', '')." <br> Modified Compactness: ".number_format(1e8*(float)$elmz[5], 2, '.', ''). "*1e-8";
		}
		echo "</div>";
		echo "</li>";
		...
		echo "Model Reconstructed Object <br> <br>Asymmetry: ".number_format((float)$elmz[6], 3, '.', '')." <br> Compactness: ".number_format((float)$elmz[7], 3, '.', '')." <br> Modified Compactness: ".number_format(1e8*(float)$elmz[8], 2, '.', ''). "*1e-8";
		}
		echo "</div>";
		echo "</li>";
...
// 				a3.trigger();
// 				a3.release();<link rel="stylesheet" href="skins/page.css" type="text/css" media="screen">
